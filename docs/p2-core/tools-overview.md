...
`src/agents/tool-policy-pipeline.ts` 实现可组合的工具策略管道：
...
`buildDefaultToolPolicyPipelineSteps()` 构建默认策略管道，按优先级依次应用：
1. `tools.profile` — profile 级策略（如 coding/messaging/minimal）
2. `tools.byProvider.profile` — provider profile 级策略
3. `tools.global` — 全局策略
4. `tools.byProvider.global` — provider 全局策略
5. `tools.agent` — agent 级策略
6. `tools.byProvider.agent` — provider agent 级策略
7. `tools.group` — group 级策略

### Tool Mutation（全新）

`src/agents/tool-mutation.ts` 实现工具定义的运行时修改：
- 允许插件在运行时修改工具的 schema、描述等
- 支持工具别名（alias）注册

### Tool Loop Detection（全新）

`src/agents/tool-loop-detection.ts` 检测工具调用循环：
- 检测 agent 是否陷入重复调用同一工具的循环
- 触发后自动中断并提示用户

### Tools Effective Inventory（全新）

`src/agents/tools-effective-inventory.ts` 计算运行时有效工具清单：
- 综合 profile、policy、plugin 注册等因素
- 返回当前 session 实际可用的工具列表

### 图像生成工具（全新）

`feat(agents): infer image generation defaults` + `feat(image-generation): add image_generate tool`：
- 新增 `image_generate` 工具（`sectionId: "media"`, `profiles: ["coding"]`）
- 通过 `src/image-generation/` 模块路由到 image generation provider（fal、google 等）
- `feat(plugins): add image generation capability` — 插件可注册 image generation provider
