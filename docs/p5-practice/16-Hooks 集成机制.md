...
### Subagent Ended Hooks 保留（重要修复）

`fix: preserve subagent ended hooks until runtime init`：
- 子代理结束时触发的 hooks 现在会保留到 runtime 初始化完成后再执行
- 防止 hooks 在 runtime 未就绪时触发导致的竞态条件

### Bootstrap Hooks（全新）

`src/agents/bootstrap-hooks.ts` — 启动钩子：
- 在 agent 启动时触发
- 用于初始化 workspace、加载配置等

### Sessions Spawn Hooks（全新）

`src/agents/sessions-spawn-hooks.ts` — 会话生成钩子：
- 在子代理生成时触发
- 允许插件在子代理启动前/后执行自定义逻辑

### Hook Pack Installs 统一

`feat(cli): unify hook pack installs under plugins`：
- hook pack 安装现在统一到 `plugins` 命令下
- 与 ClawHub 插件安装流程集成

### Hooks 模块新增

`src/hooks/` 目录新增：
- `gmail.ts` + `gmail-watcher.ts` — Gmail 集成 hook
- `llm-slug-generator.ts` — LLM slug 生成器
- `hooks-install.ts` — hook 安装管理
- `hooks-status.ts` — hook 状态查询
- `message-hook-mappers.ts` — 消息 hook 映射器