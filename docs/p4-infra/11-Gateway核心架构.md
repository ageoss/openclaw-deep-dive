...
graph TB
    subgraph transport["🌐 传输层 Transport Layer"]
        direction LR
        HTTP["📡 HTTP Server<br/><small>node:http</small>"]
        WS["🔌 WebSocket Server<br/><small>ws library</small>"]
    ...
    subgraph handlers["⚙️ 处理层 Handler Layer"]
        ReqH["🎯 Request Handlers<br/><small>105 methods</small>"]
        EvtB["📡 Event Broadcaster"]
        NodeR["🖥️ Node Registry"]
    end

    subgraph subsystems["🏛️ 核心子系统 Core Subsystems"]
        AgentE["🤖 Agent Events"]
        ChatS["💬 Chat State"]
        Canvas["🎨 Canvas Host"]
        Cron["⏰ Cron"]
    ...
    transport ==>|"帧解析"| protocol
    protocol ==>|"路由分发"| handlers
    handlers ==>|"执行调用"| subsystems

    classDef transportStyle fill:#dbeafe,stroke:#1d4ed8,stroke-width:3px,color:#1e3a8a
    classDef protocolStyle fill:#dcfce7,stroke:#15803d,stroke-width:3px,color:#14532d
    classDef handlerStyle fill:#fed7aa,stroke:#c2410c,stroke-width:3px,color:#7c2d12
    classDef subsystemStyle fill:#e9d5ff,stroke:#7c3aed,stroke-width:3px,color:#4c1d95
...
```mermaid
graph TB
    Start["① 📨 WebSocket<br/>Message Received"]
    Parse["② 🔍 Parse JSON<br/>→ RequestFrame"]
    Validate["③ ✅ Ajv Validation<br/><small>validateRequestFrame</small>"]
    Extract["④ 📦 Extract method<br/>& params"]
    Lookup["⑤ 🔎 Method Lookup<br/><small>BASE_METHODS +<br/>channelMethods</small>"]
    Resolve["⑥ 🎯 Resolve Handler"]
    BuildCtx["⑦ 🏗️ Build RequestContext<br/><small>deps, cron, nodeRegistry<br/>broadcast, chatRunState,<br/>agentRunSeq</small>"]
    Execute["⑧ ⚡ Execute Handler"]
    Respond["⑨ 📤 Handler calls respond<br/><small>ok, payload, error</small>"]
    SendResp["⑩ 📬 Send ResponseFrame"]

    ParseErr["❌ Error Response<br/><small>Parse Failed</small>"]
    ValErr["❌ Error Response<br/><small>Validation Failed</small>"]
    ...
    classDef processStyle fill:#e0e7ff,stroke:#4338ca,stroke-width:2px,color:#312e81
    classDef errorStyle fill:#fee2e2,stroke:#b91c1c,stroke-width:2px,color:#7f1d1d
    classDef startStyle fill:#d1fae5,stroke:#047857,stroke-width:2px,color:#064e3b
    classDef successStyle fill:#fef3c7,stroke:#b45309,stroke-width:2px,color:#78350f
    ...
    class Validate,Resolve,Respond successStyle
```
...
```mermaid
graph TB
    Start["🤖 Agent Event<br/><small>from Pi SDK</small>"]
    Handler["🎯 createAgentEventHandler()"]

    SeqCheck["🔢 seq gap check & update<br/><small>agentRunSeq</small>"]
    SeqErr["⚠️ 序列号不连续<br/>→ 发送 error 事件"]

    ToolQ["🔧 Tool Event?"]
    ToolAct["📡 broadcastToConnIds<br/><small>订阅者</small>"]
    NodeSend["🖥️ nodeSendToSession<br/><small>verbose=full</small>"]

    AssistQ["💬 Assistant Event?"]
    ChatDelta["⏱️ emitChatDelta<br/><small>150ms 节流</small>"]
    ChatProc["🔍 文本去重 +<br/>静默回复过滤"]

    LifeQ["🔄 Lifecycle<br/>end/error?"]
    ChatFinal["📨 emitChatFinal"]
    Cleanup["🧹 清理 chatRunState"]
    MarkFinal["✅ toolEventRecipients<br/>.markFinal"]

    Broadcast["🌐 Broadcast All Clients<br/><small>broadcast 'agent', payload</small>"]
    ...
    classDef actionStyle fill:#fce7f3,stroke:#be185d,stroke-width:2px,color:#831843
    classDef errorStyle fill:#f3f4f6,stroke:#6b7280,stroke-width:2px,stroke-dasharray: 5 5,color:#374151
...
graph TB
    ...
    classDef gatewayStyle fill:#dbeafe,stroke:#1d4ed8,stroke-width:3px,color:#1e3a8a
    classDef cronStyle fill:#e9d5ff,stroke:#7c3aed,stroke-width:3px,color:#4c1d95
    classDef presenceStyle fill:#dcfce7,stroke:#15803d,stroke-width:3px,color:#14532d
...
graph TB
    subgraph flow1["① 🔌 Node 连接 Gateway"]
        NodeReg["📝 NodeRegistry.register<br/><small>nodeId, connId</small>"]
        PresenceUpsert["🔄 upsertPresence<br/><small>presenceKey, mode: 'node'</small>"]
        NodeReg --> PresenceUpsert
    end

    subgraph flow2["② 📍 Presence 状态变化"]
        BroadcastSnap["📡 broadcastPresenceSnapshot()"]
        GatewayPres["🌐 Gateway.broadcast<br/><small>'presence', payload</small>"]
        BroadcastSnap --> GatewayPres
    end

    subgraph flow3["③ ⏰ Cron 定时触发"]
        Schedule["📅 schedule.match<br/><small>now</small>"]
        ListPresence["🔍 listSystemPresence<br/><small>过滤活跃设备</small>"]
        DeliveryTargets["📬 deliveryTargets<br/><small>决定发给谁</small>"]
        GatewayCron["🌐 Gateway.broadcast<br/><small>'cron', payload</small>"]
    ...
    subgraph flow4["④ ❌ 设备掉线"]
       WSClose["🔌 WebSocket.close"]
        ClientsDelete["🗑️ Gateway.clients<br/>.delete connId"]
        TTLExpire["⏱️ Presence TTL 过期<br/><small>5 分钟后</small>"]
        CronUpdate["🔄 Cron 收到 'presence'<br/>更新交付目标"]
    ...
    classDef flowStyle fill:#fef3c7,stroke:#b45309,stroke-width:2px,color:#78350f
    classDef gatewayStyle fill:#dbeafe,stroke:#1d4ed8,stroke-width:2px,color:#1e3a8a
    classDef presenceStyle fill:#dcfce7,stroke:#15803d,stroke-width:2px,color:#14532d
    classDef cronStyle fill:#e9d5ff,stroke:#7c3aed,stroke-width:2px,color:#4c1d95

    class NodeReg,PresenceUpsert,BroadcastSnap,GatewayPres,WSClose,ClientsDelete,TTLExpire,CronUpdate flowStyle
    class Schedule,ListPresence,DeliveryTargets,GatewayCron cronStyle
```
